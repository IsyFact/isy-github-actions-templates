...
=== Docs Build Template
Das Template `docs_build_template.yml` ist ermöglicht den automatisierten Start von Dokumentations-Builds für Antora-Projekte. Es nutzt das Event `workflow_call`, um den Workflow `antora_build.yml` im Repository `IsyFact/isyfact.github.io` über die GitHub CLI auszuführen. Dafür ist das Secret `ANTORA_TRIGGER_TOKEN` erforderlich, das die notwendigen Berechtigungen zum Authentifizieren und Auslösen des Workflows hat. Diese Automatisierung vereinfacht die Integration von Dokumentationsaktualisierungen in die CI/CD-Pipeline und sorgt für eine effiziente Bereitstellung von Dokumentationsänderungen.
