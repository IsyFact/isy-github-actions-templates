...
|===
^|Eingabe ^|Erforderlich ^|Standardwert ^|Beschreibung
|`base-ref` |Nein |`${{ github.event.pull_request.base.sha }}` .2+|Git-Referenzen für die Git-Base/den Git-Head bei der Durchführung der Vergleichsprüfung. Dies wird nur für andere Event Types als `pull_request` und `pull_request_target` verwendet
|`head-ref` |Nein |`${{ github.event.pull_request.head.sha }}` |
|===
