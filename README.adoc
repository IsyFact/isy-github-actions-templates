...
|===
^|Eingabe ^|Erforderlich ^|Standardwert ^|Beschreibung
|`jdk-version` |Nein |`25` |Version des zu verwendenden JDKs
...
|`jdk-version`              |`false`      |`25`         |JDK Version
...
=== Docs Build Template
Das Template `docs_build_template.yml` erleichtert die automatische Durchführung von Builds der Dokumentation für Antora-Projekte.
Es ist aktuell für die Erstellung von Release-Dokumentation und zum Kommentieren von Pull Requests mit Änderungen an der Dokumentation gedacht.
...
Er wartet, bis der ausgelöste Build abgeschlossen ist.
Wenn der Workflow durch einen Pull Request gestartet wurde, verarbeitet er das als Artefakt veröffentlichte Antora-Log gegen den letzten Build der produktiven Online-Dokumentation und bereitet eine Übersicht der behobenen (fixed) und neu hinzugekommenen (introduced) Log-Einträge auf.
Zusammen mit einer allgemeinen Übersicht des Builds und relevanter Log-Einträge veröffentlicht der Workflow das Ergebnis als Kommentar am Pull Request.

*Prüfung der Diagramme (draw.io)*:
Der Workflow prüft, ob Diagramme im SVG-Format den Anforderungen der Online-Dokumentation genügen.
Aktuell prüft der Workflow, ob die Diagramme adaptive Farben verwenden, da diese in der Online-Dokumentation nicht korrekt dargestellt werden können. Weiter Erläuterungen hierzu finden sich im https://isyfact.github.io/docs/documentation-guide/current/seitenbearbeitung.html#draw.io[Leitfaden Dokumentation].

*Anpassbares Logging*:
Der Workflow ermöglicht es, das Logging des Antora-Builds zu steuern:

- Parameter `log_level`: Spezifiziert den Schweregrad, ab dem eine Nachricht ins Log geschrieben wird. Weitere Details: https://docs.antora.org/antora/latest/playbook/runtime-log-level/[Antora: Log Severity Level].
- Parameter `failure_level` : Spezifiziert den Schweregrad, ab dem der Antora-Build mit einem Fehlercode (Rückgabewert ungleich 0) antwortet. Weitere Details: https://docs.antora.org/antora/latest/playbook/runtime-log-failure-level/[Antora: Log Failure Level].

*Fehlerbehandlung*:
Wenn der Antora-Build fehlschlägt (s. Parameter `failure_level`), schlägt der Workflow ebenfalls fehl.
Wenn der Workflow durch einen Pull Request gestartet wurde, kann das Repository so konfiguriert werden, dass ein fehlschlagender Antora-Build den Merge blockiert.
...
| Nein
| `info`
| https://docs.antora.org/antora/latest/playbook/runtime-log-level/[Antora: Log Severity Level]

| `failure_level`
| Nein
| `fatal`
| https://docs.antora.org/antora/latest/playbook/runtime-log-failure-level/[Antora: Log Failure Level]
...
| Ja
|
| GitHub-Token zum Auslösen des Antora-Builds
|===

==== Nutzung zur Veröffentlichung eines Releases
[source,yaml]
----
on:
  push:
...
jobs:
  Build:
    uses: IsyFact/isy-github-actions-templates/.github/workflows/docs_build_template.yml@main <.>
    secrets:
      ANTORA_TRIGGER_TOKEN: ${{ secrets.ANTORA_TRIGGER_TOKEN }}
----
<.> Für die Verwendung eines stabilen Standes muss eine Release-Version gesetzt werden.

==== Nutzung für die Kommentierung von PRs
[source,yaml]
----
on:
  pull_request:
...
Das Template `semgrep.yml` führt einen Semgrep-Sicherheitsscan durch. Es nutzt das offizielle Semgrep-Container-Image und prüft den Code auf Sicherheitslücken und Best Practices.

Das Template verwendet die Semgrep-Regeln aus dem Verzeichnis `.github/config/semgrep` dieses Repositories. Diese werden automatisch aus dem Template-Repository geladen, sodass keine lokale Kopie im aufrufenden Repository erforderlich ist.
...
jobs:
  semgrep-scan:
    uses: IsyFact/isy-github-actions-templates/.github/workflows/semgrep.yml@<version>
----
